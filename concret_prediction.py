...
def get_blast_results(cur_predictions_list1, cur_predictions_values_dic1, cur_type_list1):
    ...
    if singletons_blast_resp_list != []:
        best_allgenus_ident_info = (singletons_blast_resp_list[0][0],
                                    [float(singletons_blast_resp_list[0][1]), float(singletons_blast_resp_list[0][2])],
                                    singletons_blast_resp_list)
    else:
    ...
    for genus_fasta_path in fasta_list:
        ...
        if ident_list != []:
            ...
            if statistics.mean([float(ident_list[0][1]), float(ident_list[0][2])]) > statistics.mean(
                    best_allgenus_ident_info[1]):
                best_allgenus_ident_info = (
                (genus_fasta_path.split("."))[0], [float(ident_list[0][1]), float(ident_list[0][2])], ident_list)
    ...
    if best_allgenus_ident_info[0] != "":
        ...
        if best_allgenus_ident_info[0] != cur_predictions_list1[-1]:
            ...
            if singletons_blast_resp_list and best_allgenus_ident_info[0] == singletons_blast_resp_list[0][0]:
                ...
                cur_predictions_values_dic1[best_allgenus_ident_info[0]] = (
                [[float(best_allgenus_ident_info[1][0]) / 100]], pred_categories)
        ...
        final_blast_results.append(
            "Accession with NO sequence similarity using blast to the predicted family genuses members!")
...
def blast_genus(sequence, genus_path, dir_path1):
    ...
    seq_result_file_path = os.path.join(files_path, "blast_results_" + acc_id_forPath + "_gen_"
                                        + genusname + ".txt")
    ...
    for line in resp_file_blast:
        ...
        line = line.split("\t")
        resp_list.append((line[1], line[2], line[4]))
        if counter == BLAST_GENUS_MAX_HITS:
            break
    resp_file_blast.close()
...
########################################################################################################################
# input information
inputfile = "examples/example_test_allgenus.fasta"
model_dir_path = "models/Cressdnaviricota"
output_path = "output"

parser = argparse.ArgumentParser()

parser.add_argument('--in', dest='inputfile', default=inputfile, help='The input path to query file. Default is '
                                                                      'the example file.')
...
parser.add_argument('--out', dest='outputpath', default=output_path, help='A path for the output directory, where a '
                                         'output folder will be created. Default is the output directory.')
...
parser.add_argument('--save', action ="store_true", help='To select if you want to save the blast results files.'
                                                         ' Default is no..')
...
parser.add_argument('--gpu', action='store_true', help='Use GPU for the analysis when available. Default is using CPU.')
...
if gpu2:
    ...
    write_output(logpath,"Approximated Used GPU memory: " +str(cur_gpu_memory_usage[0] - last_gpu_usage[0]) + " MBs.\n")
    print("Approximated Used GPU memory: " + str(cur_gpu_memory_usage[0] - last_gpu_usage[0]) + " MBs.")
print("Models loaded in " + str(round(time.time() - load_time)) + " seconds. Models Disk Size: " + str(
    sum(disk_space_list)) +
      " MBs.")
...
file_time = time.time()
med_time_list = []
write_output(logpath, "There are "+str(len(fasta_test_dic))+" sequences in the file.\nParsing...\n###")
print("There are "+str(len(fasta_test_dic))+" sequences in the file.\nParsing...\n###")
for cur_seq_identifier, cur_sequence in fasta_test_dic.items():
    ...
    write_output(logpath, "Current sequence identifier:" + cur_seq_identifier + "\n")
    print("Current sequence identifier: " + cur_seq_identifier)
    ...
    write_output(logpath, "Prediction: " + str(last_taxa2) + "\n")
    print("Prediction: " + str(last_taxa2))
    ...
    print("Total sequence time taken: " + str((time.time() - idtime)) + " s;")
    write_output(logpath, "Total sequence time taken: " + str((time.time() - idtime)) + " s;\n")
    print("Total time passed: " + str((time.time() - file_time) / 60) + " min;")
    write_output(logpath, "Total time passed: " + str((time.time() - file_time) / 60) + " min;\n")
    print("Current: ", count_tot_seqs, ". Total: ", len(fasta_test_dic), "\n----------------------")

if not SAVE_FILES:
    shutil.rmtree(files_path)
print("Total number of sequences: ", len(fasta_test_dic))
write_output(logpath, "Total number of sequences: " + str(len(fasta_test_dic)) + "\n")
print("Run total time: " + str((time.time() - file_time) / 60) + " min;")
