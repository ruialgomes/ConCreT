...
from concret_gui import GUIApp
app = GUIApp()

app.inputfile.set(inputfile)
app.modelpath.set(model_dir_path)
app.outputpath.set(output_path)

app.run()

# Get the user inputs from the GUI
inputfile, modelpath, outputpath, outputname, save, max_blast_hits, gpu = (
        app.inputfile.get(),
        app.modelpath.get(),
        app.outputpath.get(),
        app.outputname.get(),
        app.save.get(),
        app.max_blast_hits.get(),
        app.gpu.get()
    )
init_test_file_path = inputfile
model_dir_path = modelpath
output_path = outputpath
output_name = outputname
SAVE_FILES = save
USE_GPU = gpu
BLAST_GENUS_MAX_HITS = max_blast_hits
gpu2 = False
if USE_GPU and gpus:
    print("Using GPU.")
    gpu2 = True
